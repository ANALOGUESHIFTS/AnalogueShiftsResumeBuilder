...
import ViewTemplateModal from "@/components/application/view-template-modal";
  ...
  return (
    <>
      ...
      <section className="w-full h-max">
        <nav className=" w-full bg-white h-max py-4 z-60 px-5 overflow-x-auto md:overflow-x-hidden">
          <div className="flex md:flex-wrap gap-4 w-max md:w-full">
            {resumeTemplates.map((data) => {
              return (
                <Link
                  key={crypto.randomUUID()}
                  style={{ border: `1px solid #4b94ea` }}
                  href={"/resume-templates#" + data.id}
